jQuery(document).ready(function ($) {
    ...
    function getWord() {
        //a. build word spans
        words.innerHTML = "";
        var wordArray = sentenceList[wordIndex].split("");
        ...
        //b. show picture
        jQuery(".game_dictation").css("background-image", "url(../wp-content/uploads/userdata" + userID + "/picture/" + wordlist_id +"_"+ wordIndex+")");



        //c. play sound
        wordSound = new Audio("../wp-content/uploads/userdata" + userID + "/paragraph/" + wordlist_id +"_"+ wordIndex+ ".mp3");
        wordSound.pause();
    ...
    function typing(e) {
        ...
        for (var j = 0; j < spans.length; j++) { //checking if all the letters are typed
            ...
            if (checker === spans.length) { // if so, animate the words with animate.css class
                ...
                setTimeout(function () {
                    if(wordIndex==sentenceList.length)
                    {
                        if(errors==0)
                        {
